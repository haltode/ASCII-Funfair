...
void lireCodeMastermind(int nbTentative, char combinaisonEntree[TAILLE_MAX])
{
	int compteur;
	...
	{
		if(compteur > 0)
			printf("Veuillez entrer une combinaison valide !\n");
	...
	} while(!estCodeBon(combinaisonEntree));
}
...
void mastermind(int modeDeJeux)
{
	...
	do
	{
		...
		lireCodeMastermind(tentative, combinaisonEntree);
...
int compterBateauBatailleNavale(int typeJoueur)
{
	int lig, col;
	int nbBateau;
	...
	for(lig = 0; lig < NB_LIGNE_BATAILLE_NAVALE; lig++)
	{
		for(col = 0; col < NB_COLONNE_BATAILLE_NAVALE; col++)
		{
			if(jeuBatailleNavale[lig][col] == typeJoueur)
				nbBateau++;
		}
	}
...
void batailleNavale(int modeDeJeux)
{
	...
	nbBateauHumain = compterBateauBatailleNavale(HUMAIN);
	nbBateauIA = compterBateauBatailleNavale(IA);
...
void enregistrerDernierePartieJoueeSolo(void)
{
	...
	/* Premierement on verifie si la partie nomPartie n'a pas ete supprimee 
	   Si ce n'est pas le cas on peut l'enregistrer */
	
	strcpy(chemin, nomDossierSauvegarde);
	strcat(chemin, "/nb_points.txt");
	
	testFichier = fopen(chemin, "r");
